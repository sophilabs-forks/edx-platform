...
def get_certificate_description(mode, certificate_type, platform_name):
    ...
    if mode == 'honor':
        ...
        certificate_type_description = _(u"An {cert_type} Certificate signifies that an {platform_name} "
...
def _update_certificate_context(context, user_certificate, platform_name):
    ...
    context['certificate_info_description'] = _(u"{platform_name} acknowledges achievements through certificates, which "
...
def _update_context_with_basic_info(context, course_id, platform_name, configuration):
    ...
    context['company_about_description'] = _(u"{platform_name} offers interactive online classes and MOOCs from the "
...
def _update_social_context(request, context, course, user, user_certificate, platform_name):
    ...
    context['twitter_share_text'] = share_settings.get(
        ...
        _(u"I completed a course on {platform_name}. Take a look at my certificate.").format(
            platform_name=platform_name
        )
    )
...
def _update_context_with_user_info(context, user, user_certificate):
    ...
    context['accomplishment_banner_opening'] = _(u"{fullname}, you've earned a certificate!").format(
